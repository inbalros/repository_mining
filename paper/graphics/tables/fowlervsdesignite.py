...
for project in projects:
    ...
    try:
        ...
        designite_path = Config.get_work_dir_path(os.path.join("paper", "analysis", "designite", project, "scores.csv"))
...
values = {c: np.mean(list(map(lambda x: x[c[2]], scores[c[0]][c[1]]))) for c in list(product(datasets, criterion, metrics))}
...
df.to_csv("fowlervsdesignite.csv", index=False)
